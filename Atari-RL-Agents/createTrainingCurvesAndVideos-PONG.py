...
def run_episodes_and_create_video(policy, eval_tf_env, eval_py_env, label):
    ...
    gif_file_name = f'myAgentPlays-PONG-{label.upper()}.gif'
...
def get_sorted_pol_list(parent_dir):
    ...
    num_policies_to_select = 10
    step = len(policyList) // num_policies_to_select
...
def main(argv):
    ...
    plt.savefig('trainingCurve-PONG.Apng')
